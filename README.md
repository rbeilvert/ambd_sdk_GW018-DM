...
After a firmware write the module stays in the Gecko bootloader instead of
starting the application, and `probe` reports `GECKO_BOOTLOADER, version
'1.9.2'`. This applies to the write only. The module starts normally on
later power cycles. Start the application by sending `2` to the bootloader
menu over the same socket:
